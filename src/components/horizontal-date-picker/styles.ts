...
export const StyledDateChip = styled.button<{ $active: boolean }>`
  display: flex;
  ...
  border: ${({ $active }) => ($active ? "0.125rem" : "0.0625rem")} solid
    ${({ $active, theme }) =>
      $active ? theme.palette.primaryMain : theme.palette.outline};
  border-radius: 1rem;
